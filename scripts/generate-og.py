...
def main():
    ...
    y_title = 214
    d.text((PAD, y_title), "Edge", font=f_title, fill=WHITE)
    edge_w = d.textlength("Edge", font=f_title)
    d.text((PAD + edge_w, y_title), "Sounds", font=f_title, fill=ACCENT)

    d.text((PAD, 352), "Make your FPV radio talk back.", font=font(HELV, 40), fill=ZINC_300)

    # Spec strip, bottom-left, with dot separators.
    f_mono = font(MENLO, 23)
    x, y = PAD, 534
    parts = [("32 kHz", ACCENT), ("mono", ZINC_500), ("16-bit PCM", ACCENT),
